# Agent instructions for EVM Wallet Generator

These instructions apply to this repository. Follow the user's requested scope;
use only the unified CLI documented in README.md. This is an offline EVM wallet
...
- Commands: `generate`, `derive`, `verify`, `pack`.
...
- Never include the mnemonic in the ZIP. The ZIP contains encrypted V3 keystores
  and public metadata; it is not itself an encrypted container.

## Operating workflow

1. Confirm the intended count, output directory and, for derive, start index from
...
6. Report counts, public paths and verification outcome. Do not claim online import
...
- `bin/` contains the executable; `src/cli.js` owns command options;
  `src/commands/` owns workflows; `src/chains/evm.js` owns EVM derivation;
